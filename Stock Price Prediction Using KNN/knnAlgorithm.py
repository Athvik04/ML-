...
def loadDataset(filename, split, trainingSet=None, testSet=None, content_header=None):
    trainingSet = trainingSet if trainingSet is not None else []
    testSet = testSet if testSet is not None else []
    content_header = content_header if content_header is not None else []
...
def euclideanDistance(instance1, instance2, length):
    distance = sum(pow((instance1[x] - instance2[x]), 2) for x in range(1, length))
    return math.sqrt(distance)
...
def getNeighbors(trainingSet, testInstance, k):
    ...
    for x in range(len(trainingSet)):
        ...
        distance.append((trainingSet[x], dist))
    distance.sort(key=operator.itemgetter(1))
    return [distance[x][0] for x in range(k)]
...
def getAccuracy1(testSet, predictions):
    correct = sum(1 for x in range(len(testSet)) if RMSD(testSet[x][-1], predictions[x]) < 1)
    return (correct / float(len(testSet))) * 100.0
...
def change(today, yest):
    return 'up' if today > yest else 'down'
...
def getData(filename, stockname, startdate, enddate):
    ...
    with open(filename, 'w', newline='') as pp:
        stockwriter = csv.writer(pp)
        first_time = True
        for i in sorted(stck_dates.keys()):
            new_format_date = i[:10]
            if first_time:
                first_time = False
                prev_closing = stck_dates[i]["Adj Close"]
...
def predict_and_get_accuracy(testSet, trainingSet, k, stockname):
    ...
    plt.figure()
    plt.title(f"Prediction vs Actual Trend of {stockname}")

    row, col = zip(*[(datetime.datetime.strptime(test[0], "%Y-%m-%d"), 1 if pred == "up" else -1) for test, pred in zip(testSet, predictions)])
    plt.plot(row, col, 'r', label="Predicted Trend")

    row, col = zip(*[(datetime.datetime.strptime(test[0], "%Y-%m-%d"), 1 if test[-1] == "up" else -1) for test in testSet])
    plt.plot(row, col, 'b', label="Actual Trend")
...
def main():
    ...
    predictFor(5, 'amtd.csv', 'AMTD', startdate, enddate, True, split)
    predictFor(5, 'amazon.csv', 'AMZN', startdate, enddate, True, split)
    predictFor(5, 'disney.csv', 'DIS', startdate, enddate, True, split)
    predictFor(5, 'sbux.csv', 'SBUX', startdate, enddate, True, split)
    predictFor(5, 'twlo.csv', 'TWLO', startdate, enddate, True, split)
    predictFor(5, 'twtr.csv', 'TWTR', startdate, enddate, True, split)
    predictFor(5, 'yahoo.csv', 'YHOO', startdate, enddate, True, split)
